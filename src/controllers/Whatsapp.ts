...
class WhatsappMessenger extends Messenger {
  ...
  constructor(
    ...
    this.client.on('auth_failure', (message) => {
      // Write to terminal when auth fail
      console.log('auth_failure', message)
    })
    this.client.on('disconnected', (message) => {
      // Write to terminal when disconnected
      console.log('disconnected', message)
    })
    this.client.on('ready', () => {
      // Write to terminal when module is ready
      this.ready = true
      console.log('WhatsApp module is now ready to use.')

      if (this.profileName) this.client.setDisplayName(this.profileName)
      if (this.profileStatus) this.client.setStatus(this.profileStatus)
    })
  }

  validateSessionName(sessionName: string): string {
    let regex = /[ ]/gm
    if (regex.test(sessionName)) return sessionName.replace(regex, '')
  ...
  override sendMessage(chatId: string, message): void {
    if (!this.isReady()) throw new Error('Messenger is not ready.')

    app.messenger.client.sendMessage(chatId, message)

    console.log(
      `Message sent\n\n     From:   ${
        this.profileName || 'This session'
      }\n     To: ${chatId}\n     Body:   ${message}`
    )
  }
}
