...
router.get('/:login', (req, res)=>{                // Get user by id
        ...
        if (userData.email) {
            userController.getByEmail(userData.email).then(userWithEmail=>{
                ...
                user.create().then(createdUser=>{
                    return res.status(httpCodes.RESOURCE_CREATED).json(createdUser.getData())
                })
            })
        } else
            user.create().then(createdUser=>{
...
router.put('/:login', (req, res)=>{                        // Update one user by id
    ...
    userController.getByLogin(username).then(user=>{
        ...
        if(!hasChanges(user.getData(true), req.body))               // Verify changes
            return res.status(httpCodes.BAD_REQUEST).json({
                error: "No changes"
            })
...
router.delete('/:login', (req, res)=>{                     // Delete one user by id
    let username = req.params.login?.trim()
